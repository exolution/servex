...
function Filter(name, handler, annotation) {
    ...
    this.disabled=!Config.servex.debug&&annotation.debug||Config.servex.debug&&annotation.release;
    this.annotation=annotation;
}
Filter.prototype.run = function(request, response, next, invokeContext) {
    var p = new Promise();
    var instace=new FilterInstance(request,response);
    Utils.executeAsyncGeneratorFunc(this.handler, instace, [request, response, next, invokeContext], p)
    return p;
};
