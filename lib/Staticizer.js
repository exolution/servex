...
exports.refresh = function(action, uri) {
    ...
    var staticPath = Path.join(Config.get('servex').projectPath, Config.get('servex').staticDir, action.name);
...
exports.init = function(actionContext) {
    for(var actionName in actionContext) {
        var staticPath = Path.join(Config.get('servex').projectPath, Config.get('servex').staticDir);
        if(!Fs.existsSync(staticPath)) {
            console.log('make static cache dir:', staticPath);
        ...
        if(actionContext.hasOwnProperty(actionName)) {
            ...
            if(action.meta.cache) {
                ...
                if(!Fs.existsSync(actionStaticPath)) {
                    console.log('make dir for action cache:', actionStaticPath);
...
function resolveStaticFilePath(action, uri) {
    ...
    return Path.join(Config.get('servex').projectPath, Config.get('servex').staticDir, action.name, Utils.FileNameEncoder.encode(Url.parse(uri).path) +
                                                                                                    '.html');
}
