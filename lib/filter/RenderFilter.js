...
exports.filter = function *(request, response, next, context) {
    ...
    if(context.actionInstance) {
        if(context.actionInstance.view) {
            var viewPath = Path.join(Config.get('servex').projectPath, Config.get('servex').viewDir, context.actionInstance.view +
                                                                                                     '.html');
            Fs.createReadStream(viewPath).pipe(context.readyStream);
            context.readyStream.statusCode = 200;
            context.readyStream.setHeaders = ('content-type' , 'text/html;charset=utf-8');
            if(Config.servex.templateEngine) {
