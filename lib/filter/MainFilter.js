...
exports.filter = function *main(request, response, next, context) {
    ...
    if(match) {
        ...
        if(match.router.maybeStaticFile(parsedUrl.pathname)) {
            ...
            console.log('but this request path like a static file!');
            var hasFile = yield tryFile(filename);
            if(!hasFile) {
                console.log('tryfile[' + filename + ']:Not found! run action!')
                ...
                console.log('tryfile[' + filename + ']:find! static file first!');
