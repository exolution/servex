...
exports.filter = function *main(request, response, next, context) {
    ...
    if(match) {
        ...
        if(match.router.maybeStaticFile(parsedUrl.pathname)) {
            var filename = Path.join(Config.get('servex').assetsDir, parsedUrl.pathname);
            console.info('but this request path like a static file!');
            var hasFile = yield tryFile(filename);
            if(!hasFile) {
                console.info('tryfile[' + filename + ']:Not found! run action!')
                ...
                console.info('tryfile[' + filename + ']:find! static file first!');
