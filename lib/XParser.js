...
function _parse(html) {
    ...
    while(result = tagSniper.exec(html)) {
        ...
        if(result[4] === undefined) {
            ...
            if(!curNode) {
                root.push(new TextNode(result[0]))
            }
            else if(curNode.tagName !== result[4].toLowerCase()) {//开始和结束标签不匹配 作为文本节点处理
                nodeStack.push(curNode);
...
function render(node, ctx, isHidden, isIgnore) {
    if(node.textContent !== undefined) {//渲染文本节点
        ...
        if(!isIgnore) {//不是忽略状态 进行各种avalon指定的吻戏
            ...
            html = node.body;
            if(node.html) {
                html += ' ms-html="' + node.html.replace(re_trimBrace, '$1').replace(/"/g, "'") + '"';
            }
            if(isSelfClosedTag) {
                html += '/>';
            }
