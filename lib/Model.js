...
Model.prototype.updateOrSave=function*(model){
    var keyMap={};
    for(var k in this._model.allProperties){
        if(this._model.allProperties.hasOwnProperty(k)) {
            if(this._model.allProperties[k].key) {
                keyMap[k]=true;
            }
        }
    }
    ...
    var ret=(yield this.find(q))[0];
    if(ret){
        Utils.merge(ret,model,true);
        return Utils.yieldable(ret.save,ret);
    }
    else {
        console.log(111)
...
function wrapper(fn) {
    return function() {
        ...
        var args = slice.call(arguments);
        var thisModel = this;
        Utils.executeAsyncGeneratorFunc(fn, thisModel, args, promise);
        return promise;
    }
}
...
exports.runTestCase = function(testCase, dataBaseConfig) {
    ...
    Orm.connect(connector, function(err, db) {
        ...
        Utils.executeAsyncGeneratorFunc(testCase, db, [model], promise);
    });
