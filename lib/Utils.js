...
global.debug=function debug(){
    if(Config.server.debug) {
        var args = slice.call(arguments).map(function(e){
            if(typeof e ==='object'){
                return Utils.clone(Utils.isArray(e)?[]:{},e);
            }
...
Utils.wrapGenerator=function(func,thisArgs){
    ...
    return function(){
        ...
        Utils.executeAsyncGeneratorFunc(func,thisArgs,args,promise);
        return promise;
    }
}
var s_space='                          ';
...
var s=Utils.formatSql("SELECT oc.need_pick_up,               oc.name as operating_company,               oc.email as operating_company_email         FROM ht_operation_company oc         LEFT JOIN ht_order_product op ON op.product_id = oc.product_id AND oc.special_code in ('0')         WHERE op.order_id=16668");
