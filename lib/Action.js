...
var re_excludeSpecial = /Request|Response|Action|Error|^\$/;
var resolveRequestParam = function(paramList) {
    return paramList.filter(function(param) {
        return !re_excludeSpecial.test(param);
    }).map(function(param) {
        return '{' + (param[0] === '_' ? param.slice(1) : param) + '}'
    }).join('-');
};
var Action = module.exports = function(name, handler, meta) {
    ...
    this.disabled=!Config.servex.debug&&meta.debug;
...
var initArguments = function(request, response, invokeContext, actionContext) {//依赖注入
    ...
    var routeInfo = invokeContext.routeInfo;
    ...
    this.paramList.forEach(function(param) {
        ...
        if(param === 'Request') {
            ...
            argumentList.push(require('./View'));
        }
        else if(param === 'Error') {
            ...
            param = param.slice(1);
            argumentList.push(selectValue(param, query));
        }
        else {
            argumentList.push(selectValue(param, routeInfo, query,request.body ,actionContext));
        }
...
function ActionInstance(action, request, response, invokeContext) {
...
Action.prototype.newInstance = function(request, response, invokeContext) {
    return new ActionInstance(this, request, response, invokeContext);
}
Action.prototype.run = function(request, response, invokeContext) {
    ...
    Utils.executeAsyncGeneratorFunc(this.handler, instance, argumentList, promise);
    return next;
...
Action.prototype.getInvokeChain = function() {
    var list = [];
    if(this.meta.cache) {
        list.push('$staticCache');
    }
    list.push('$render');
    if(this.meta.filterList) {
        list.push.apply(list, this.meta.filterList);
    }

    while(filterName = list.pop()) {
        ...
        this.invokeChain.unshift(filter);
    }
};
